...
const ElementIcon = (props) => {
    let classString = "fas fa-" + props.type + " p-2";
    if(props.returnFunction) {
        return <i className={classString} title={props.type} onClick={() => props.returnFunction(props.params)} />
    } else {
        return <i className={classString} title={props.type} />
    }
    // switch (props.type) {
    //     case 1:
    ...
    //     case 10:
    //         return <i className="fas fa-plus p-2" title="Add Element" />
    //     default:
